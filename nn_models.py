...
class SimpleNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_hidden_layers):
            ...
            #print(x.shape)
        out = self.layer_list[self.num_hidden_layers](x)
        #print(out.shape)
        return out
...
class ComplexNN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_hidden, layer_size, dropout):
        ...
        self.layer_list.append(nn.Linear(1, self.fc_layer_neurons))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_hidden_layers):
            ...
            x = F.tanh(self.batch_norm_layer[i](self.layer_list[i](x)))
        x = self.lin_out(x)
        return x
